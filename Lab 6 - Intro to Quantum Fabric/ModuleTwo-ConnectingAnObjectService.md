...
13.	Expand the project services and click the ProdDescription microservice we created.
...
14.	Drag the get operation onto your terms and conditions form. 
 
15.	Once you drag the get operation on the screen, Visualizer will give you 3 options for mapping the data. The first option will autogenerate a List & Details template to show the data, the second will present the data in an autogenerated grid layout. The third options will let us map the data from the response to the form. Choose Detail Using Response.
j.	Now, drag the detailedDescription from the data panel into the first text box under the Advance Checking Label.
 
k.	Now drag the termsConditions attribute onto the second text box.
l.	If you want to see the client side workflow generated by Visualizer, select the properties in the right hand tab.
 
m.	Select Edit on the onMapping action. This will bring up the workflow.
 
n.	You can see that Visualizer created a number of workflow steps. If you want to edit any of the steps, you can easily edit the workflows from here.
o.	Hit cancel or save. Now, let’s look at our app in Action.

9	Run the App in Live Preview
p.	From the top menu, select Build-> Live Preview Settings
 
q.	Ensure the responsive web checkbox is selected
 
r.	Click Save and Run.
s.	This will locally compile your app as a responsive web app and start the preview browser.
 
t.	Login to the application suing the credentials you created in the first part of this lab in your user repository.
u.	If you are successful, you will successfully navigate to the dashboard form. 
v.	From the dashboard, click the advertisement, and it will load the terms page with the text fields filled with data from our object service.
